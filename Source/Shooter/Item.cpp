...
void AItem::SetActiveStars()
{
	// The 0 element isn't used in this one, 
	for (int32 i = 0; i <= 5; i++)
	{
		ActiveStars.Add(false);
	}
...
void AItem::ItemInterp(float DeltaTime)
{
	...
	if (Character && ItemZCurve)
	{
		...
		const float InterpXValue = FMath::FInterpTo(
			CurrentLocation.X,CameraInterpLocation.X,DeltaTime,30.0f);
...
void AItem::OnConstruction(const FTransform& Transform)
{
	...
	if (MaterialInstance)
	{
		...
		DynamicMaterialInstance->SetVectorParameterValue(TEXT("FernelColor"), GlowColor);
...
void AItem::UpdatePulse()
{
	...
	if (DynamicMaterialInstance)
	{
		...
		DynamicMaterialInstance->SetScalarParameterValue(TEXT("FernelExponent"), CurveValue.Y * FernelExponent);
		DynamicMaterialInstance->SetScalarParameterValue(TEXT("FernelReflectFraction"), CurveValue.Z * FernelReflectFraction);
	}
}
